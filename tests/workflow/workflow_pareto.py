...
block_2 = ModelMethod(method_type=MethodType(HeterogeneousList, 'pareto_points'), name='Pareto points')
block_3 = ModelMethod(method_type=MethodType(HeterogeneousList, 'get_attribute_values'), name='costs_1')
block_4 = ModelMethod(method_type=MethodType(HeterogeneousList, 'get_attribute_values'), name='costs_2')
block_5 = Sequence(number_arguments=2, name='list of costs')
...
pipe_3 = Pipe(block_1.outputs[0], block_4.inputs[0])
pipe_4 = Pipe(block_3.outputs[0], block_5.inputs[0])
pipe_5 = Pipe(block_4.outputs[0], block_5.inputs[1])
pipe_6 = Pipe(block_5.outputs[0], block_2.inputs[1])
pipes = [pipe_0, pipe_1, pipe_2, pipe_3, pipe_4, pipe_5, pipe_6]

workflow = Workflow(blocks, pipes, output=block_2.outputs[0], name='Pareto')

# Workflow run
workflow_run = workflow.run({
    workflow.index(block_0.inputs[0]): stream_file,
    workflow.index(block_3.inputs[1]): 'weight',
    workflow.index(block_4.inputs[1]): 'mpg'})
