...
class HeterogeneousList(DessiaObject):
    ...
    **Built-in methods**:
    ...
    def _parallel_plot_attr(self):
        ...
        #     best_graph_path.append()
        #     best_graph_path[-1] = [best_graph_path[-1], best_graph_path[-1].length]
        # ordered_attributes = next(best_graph_path)[:2]
        # for edge in best_graph_path:
        #     list_edge = list(edge)
        #     if ordered_attributes[0] in edge:
        #         list_edge.remove(ordered_attributes[0])
        #         ordered_attributes = (list_edge[0],) + ordered_attributes
        #     elif ordered_attributes[-1] in edge:
        #         list_edge.remove(ordered_attributes[-1])
        #         ordered_attributes += (list_edge[0],)
        # attr_series = self._get_attribute_trios(sorted_r2, sorted_association)
        return nx.dag_longest_path(correlation_graph, weight='weight') # self._trios_list_to_parallel_axes(attr_series)
    ...
    def _get_correlations(self):
        ...
        return zip(*sorted(zip(r2_scores, association_list))[::-1])
