"""
This is a module that aims to list all possibilities of data formats offered by Dessia.
It should be used as a repertory of rules and available typings. Some general rules :

- Lists are homogeneous, ie, they should not contain several types of elements
    ex : List[int], List[str], List[CustomClass], ...
- Tuples can, therefore, be used as heterogenous sequences, thanks to the fact that they are immutables.
    ex : Tuple[str, int, CustomClass] is a tuple like this : t = ('tuple', 1, custom_class_object)
- Dict is used whenever a dynamic structure should be defined.
  It takes only one possible type for keys and one possible type for values.
    ex : Dict[str, bool] is a dict like : d = {'key0': True, 'key1': False, 'another_key': False,...}
- As opposed to this, TypedDict defines a static structure, with a defined number of given, expected keys
...
this module can also be seen as a template for Dessia's coding/naming style & convention.
"""
...
class EnhancedStandaloneSubobject(StandaloneSubobject):
    def __init__(self, floatarg: float, boolarg: bool, name: str = 'Standalone Subobject'):
...
class EmbeddedSubobject(DessiaObject):
    def __init__(self, name: str = 'Embedded Subobject'):
...
class StandaloneObject(DessiaObject):
    ...
    def __init__(self, standalone_subobject: StandaloneSubobject, embedded_subobject: EmbeddedSubobject,
                 ...
                 builtin_list: List[int], union_arg: Union[StandaloneSubobject, EnhancedStandaloneSubobject],
    ...
    def add_float(self, value) -> StandaloneSubobject:
        """
        This methods adds value to its standalone subobject floatarg property and returns it.
